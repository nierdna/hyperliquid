...
class CrawlerService {
  ...
  private handleTrades(data: TradesData["data"]): void {
    ...
    // Lọc các giao dịch có giá trị notional >= ngưỡng cấu hình
    const filteredTrades = data.filter((trade) => {
      // Tính toán giá trị notional (px * sz)
      const px = parseFloat(trade.px);
      const sz = parseFloat(trade.sz);
      const notional = px * sz;

      // Kiểm tra nếu notional >= ngưỡng
      return notional >= config.tradeNotionalThreshold;
    });

    // Nếu không có giao dịch nào thỏa mãn điều kiện, không lưu
    if (filteredTrades.length === 0) {
      logger.debug(
        `No trades with notional >= ${config.tradeNotionalThreshold} for ${coin}`
      );
      return;
    }

    // Lưu các giao dịch đã lọc
    logger.debug(
      `Saving ${filteredTrades.length}/${data.length} trades with notional >= ${config.tradeNotionalThreshold} for ${coin}`
    );
    storage.saveCoinData("trades", coin, filteredTrades);
  }
  ...
  private handleUserNonFundingLedgerUpdates(
    data: UserNonFundingLedgerUpdatesData["data"]
  ): void {
    const user = data.user;
